...
**               Its main function is to perform the decoding of ciphertext.
...
/* decrypts the communication */
void decrypter(char*,char*,int);
...
void connector(int establishedConnectionFD) {
  ...
  while(charsRead < sizeof(buffer)) {
    ...
    if(temp==-1) {
      error("Handshake failed in otp_dec_d.c | recv returned -1");
    }
    /* store in buffer */
  ...
  /* if otp_enc tries to use otp_dec_d inform of error */
  if(strncmp(buffer,"otp_enc",7)==0) {
    ...
    while(charsWritten < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Failed in otp_enc cannot use otp_dec_d | send returned -1");
      }
      charsWritten += temp;
    }
  ...
  /* else if something weird happened */
  else if(strncmp(buffer,"otp_dec",7)!=0) {
    memset(buffer,'\0',sizeof(buffer));
    for(i=0;i<17;i++) {
      buffer[i] = failure[i];
    }
    while(charsWritten < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Failed to send BAD INPUT in otp_enc_d.c | send returned -1");
      }
      charsWritten += temp;
    }
    error("otp_dec_d received bad input as handshake...\n");
  }

  /* else if handshake successful */
  else {

    /* inform of successful handshake */
    memset(buffer,'\0',sizeof(buffer));
    ...
    while(charsWritten < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Failed to send handshake in otp_dec_d.c | send returned -1");
      }
      charsWritten += temp;
    }
    ...
    while(charsRead < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Msg length recv failed in otp_dec_d.c | recv returned -1");
      }
      /* store in buffer */
      for(i=charsRead;i < (charsRead + temp); i++) {
        temp_buffer[i] = buffer[i];
    ...
    while(charsRead < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Msg recv failed in otp_dec_d.c | recv returned -1");
      }
      /* store in buffer */
      for(i=charsRead;i < (charsRead + temp); i++) {
        temp_buffer[i] = buffer[i];
    ...
    while(charsRead < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("key length recv failed in otp_dec_d.c | recv returned -1");
      }
      /* store in buffer */
      for(i=charsRead;i < (charsRead + temp); i++) {
        temp_buffer[i] = buffer[i];
    ...
    while(charsRead < sizeof(buffer)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("key recv failed in otp_dec_d.c | recv returned -1");
      }
      /* store in buffer */
      for(i=charsRead;i < (charsRead + temp); i++) {
        temp_buffer[i] = buffer[i];
    ...
    decrypter(message,key,msg_length);
    ...
    while(charsWritten < sizeof(message)) {
      ...
      /* if temp ever equals -1 an error occurred */
      if(temp==-1) {
        error("Failed to send decrypted message | send returned -1");
      }
      charsWritten += temp;
    }
    charsWritten = 0;
...
void decrypter(char* message, char* key, int length) {
  ...
  for(i=0;i<length;i++) {
    ...
    /* if not a newline */
      ...
      /* subtract key char from message char 
       * and module by 27 (alphabet + ' ' = 27) */
      int decrypted = (m - k + 27) % 27;

      /* store each decrypted char back into message */
      const char* alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";
      message[i] = alphabet[decrypted];
    } 
  } 
