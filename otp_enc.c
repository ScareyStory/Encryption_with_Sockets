...
** Description:  This program connects to otp_enc_d 
**               And asks otp_enc_d to perform a one-time pad style encryption
**               By itself, otp_enc doesn’t do the encryption, otp_enc_d does.
**               otp_enc receives ciphertext back from otp_enc_d  
**               And outputs the ciphertext to stdout.
*******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
/* connect to otp_enc_d and send message for encryption */
...
int main(int argc, char *argv[])
{
  ...
  /* check that key is at least as long as message to encode */
  int enc_length = num_of_chars(argv[1]);
  int key_length = num_of_chars(argv[2]);
  if(enc_length > key_length) {
  ...
  /* otp_enc_d.c will look for this message to be sent over */
  char handshake[SIZE];
  memset(handshake,'\0',sizeof(handshake));
  char mini_buffer[] = "otp_enc";
  int i;
  for(i=0;i<7;i++) {
  ...
  while(charsWritten < sizeof(handshake)) {
    ...
    if(temp==-1) {
      error("Handshake send call failed in otp_enc.c");
    }
    charsWritten += temp;
  }
  ...
  while(charsRead < sizeof(handshake)) {
    ...
    /* if temp ever equals -1 an error occurred */
    if(temp==-1) {
      error("Handshake recv call failed in otp_enc.c");
    }
    /* store in buffer */
  ...
  connector(argv[1],socketFD,enc_length);
  connector(argv[2],socketFD,key_length);

  /* receive encrypted file back */
  charsRead = 0;
  temp = 0;
  memset(buffer,'\0',sizeof(buffer));
  ...
  while(charsRead < sizeof(buffer)) {
    ...
    /* if temp ever equals -1 an error occurred */
    if(temp==-1) {
      error("otp_enc failed to read in encrypted file");
    }
    /* store in buffer */
    for(i=charsRead;i < (charsRead + temp); i++) {
  ...
  /* buffer now holds encrypted message send it to stdout */
  printf("%s\n",buffer);
...
/* reads in unencrypted message and sends it to otp_enc_d for encryption */
void connector(char* filename,int socketFD,int length) {

  /* open file passed in */
  FILE* fptr = fopen(filename,"r");

  /* holds messages being passed through otp_enc */
  char buffer[SIZE];
  memset(buffer,'\0',sizeof(buffer));
  char temp_buffer[SIZE];
  memset(temp_buffer,'\0',sizeof(temp_buffer));
  ...
  /* send over message length to otp_enc_d.c */
  charsWritten = 0;
  ...
  while(charsWritten < sizeof(char_length)) {
    ...
    /* if temp ever equals -1 an error occurred */
    if(temp==-1) {
      error("otp_enc failed to send length array");
    }
    charsWritten += temp;
  }
  ...
  while(charsWritten < sizeof(buffer)) {
    ...
    /* if temp ever equals -1 an error occurred */
    if(temp==-1) {
      error("otp_enc failed to write message to otp_enc");
    }
    charsWritten += temp;
  }
